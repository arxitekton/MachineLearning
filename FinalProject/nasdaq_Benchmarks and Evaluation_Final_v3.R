...
# Read CSV into R
indices = read.csv(file="indices.csv", as.is = 1)
...
typeof(test)
tail(test)
colnames(test)[colnames(test) == 'Trade Date'] <- "dates"
colnames(test)[colnames(test) == 'Index Value'] <- "values"
test <- test[,c("dates","values")]
...
for (i in 1:num_of_indices){
  ...
  colnames(quandl_data)[colnames(quandl_data) == 'Trade Date'] <- "dates"
  colnames(quandl_data)[colnames(quandl_data) == 'Index Value'] <- "values"
  quandl_data$dates <- as.Date(quandl_data$dates, format = "%Y-%m-%d")
  #data <- data[with(data, order(data$dates, decreasing = FALSE)),]
  quandl_data <- quandl_data[,c("dates","values")]
...
print("########### MAPE ################")
for (i in 1:length(Names_benchmarks)){
  print(paste(Names_benchmarks[i], round(mean(Total_mape[i,,]), 4)))
}
